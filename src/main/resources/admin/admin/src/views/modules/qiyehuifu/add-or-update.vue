<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="回复标题" prop="huifubiaoti">
					<el-input v-model="ruleForm.huifubiaoti" placeholder="回复标题" clearable  :readonly="ro.huifubiaoti"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="回复标题" prop="huifubiaoti">
					<el-input v-model="ruleForm.huifubiaoti" placeholder="回复标题" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="回复内容" prop="huifuneirong">
					<el-input v-model="ruleForm.huifuneirong" placeholder="回复内容" clearable  :readonly="ro.huifuneirong"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="回复内容" prop="huifuneirong">
					<el-input v-model="ruleForm.huifuneirong" placeholder="回复内容" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.huifutupian" label="回复图片" prop="huifutupian">
					<file-upload
						tip="点击上传回复图片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.huifutupian" label="回复图片" prop="huifutupian">
					<img v-if="ruleForm.huifutupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.huifutupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.huifutupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
...
export default {
	...
	if(this.ruleForm.huifutupian!=null) {
		this.ruleForm.huifutupian = this.ruleForm.huifutupian.replace(new RegExp(this.$base.url,"g"),"");
	}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `qiyehuifu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.qiyehuifuCrossAddOrUpdateFlag = false;
						 this.parent.search();

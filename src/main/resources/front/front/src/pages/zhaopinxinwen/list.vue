...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"width":"100%","padding":"10px","alignItems":"center","flexWrap":"wrap","display":"flex","height":"auto"}'>
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>新闻标题</div>
        <el-input v-model="formSearch.xinwenbiaoti" placeholder="新闻标题" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>标题类型</div>
        <el-input v-model="formSearch.biaotileixing" placeholder="标题类型" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('zhaopinxinwen','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#d3ffe4","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/zhaopinxinwenAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"0 0 10px"}'>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","display":"flex","height":"auto"}'>
			<div :style='{"padding":"5px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"10px 1%","background":"#ffff","flexDirection":"row-reverse","display":"flex","width":"31%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"50%","objectFit":"cover","display":"inline-block","height":"150px"}' v-if="item.fengmiantupian && item.fengmiantupian.substr(0,4)=='http'" :src="item.fengmiantupian" class="image" />
				<img :style='{"width":"50%","objectFit":"cover","display":"inline-block","height":"150px"}' v-else :src="baseUrl + (item.fengmiantupian?item.fengmiantupian.split(',')[0]:'')" class="image" />
				<div class="item-info" :style='{"width":"280px","padding":"10px","overflow":"hidden","flexDirection":"column","display":"flex","height":"auto"}'>
					<div :style='{"lineHeight":"20px","fontSize":"14px","overflow":"hidden","color":"#333","height":"20px"}' class="name ">{{item.xinwenbiaoti}}</div>
					<div :style='{"lineHeight":"20px","fontSize":"14px","overflow":"hidden","color":"#333","height":"20px"}' class="name ">{{item.biaotileixing}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.biaotileixing != '') searchWhere.biaotileixing = '%' + this.formSearch.biaotileixing + '%';
        params['sort'] = 'faburiqi';
        params['order'] = 'desc';
        this.$http.get('zhaopinxinwen/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/zhaopinxinwenDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>

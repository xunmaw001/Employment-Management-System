...
<div :style='{"width":"1000px","padding":"20px","margin":"10px auto","position":"relative","borderRadius":"20px","background":"#fff"}'>
    <el-form
      ...
      label-width="110px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#f6f6f6"}' label="新闻标题" prop="xinwenbiaoti">
            <el-input v-model="ruleForm.xinwenbiaoti" 
                placeholder="新闻标题" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#f6f6f6"}' label="标题类型" prop="biaotileixing">
            <el-input v-model="ruleForm.biaotileixing" 
                placeholder="标题类型" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#f6f6f6"}' label="封面图片" v-if="type!='cross' || (type=='cross' && !ro.fengmiantupian)" prop="fengmiantupian">
            <file-upload
            tip="点击上传封面图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#f6f6f6"}' class="upload" v-else label="封面图片" prop="fengmiantupian">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#f6f6f6"}' label="发布人" prop="faburen">
            <el-input v-model="ruleForm.faburen" 
                placeholder="发布人" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#f6f6f6"}' label="新闻内容" prop="xinwenneirong">
            <editor 
...
<script>
  export default {
    data() {
      return {
        ...
        rules: {
          ...
          biaotileixing: [
            { required: true, message: '标题类型不能为空', trigger: 'blur' },
          ],
          fengmiantupian: [
          ],
          xinwenneirong: [
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='biaotileixing'){
              this.ruleForm.biaotileixing = obj[o];
              this.ro.biaotileixing = true;
              continue;
            }
            if(o=='fengmiantupian'){
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('zhaopinxinwen/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('zhaopinxinwen/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('zhaopinxinwen/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
